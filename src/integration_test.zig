...
test "decoder produces identical output to JavaScript reference" {
    ...
    const test_data = std.fs.cwd().readFileAlloc(allocator, "quic_data.bin", 10 * 1024 * 1024) catch |err| switch (err) {
        error.FileNotFound => {
            std.debug.print("⚠️  Test file 'quic_data.bin' not found, skipping integration test\n", .{});
            return;
        },
        else => return err,
    };
...
test "decoder preserves image dimensions and format" {
    ...
    const test_data = std.fs.cwd().readFileAlloc(allocator, "quic_data.bin", 10 * 1024 * 1024) catch |err| switch (err) {
        error.FileNotFound => {
            std.debug.print("⚠️  Test file 'quic_data.bin' not found, skipping dimension test\n", .{});
            return;
        },
        else => return err,
    };
    ...
    // Expected values for our test file
    try testing.expectEqual(@as(u32, 2048), decoder.width);
    try testing.expectEqual(@as(u32, 1152), decoder.height);

    // The test file might be RGB24 or RGB32
    const is_rgb24 = decoder.image_type == quic.Constants.QUIC_IMAGE_TYPE_RGB24;
    const is_rgb32 = decoder.image_type == quic.Constants.QUIC_IMAGE_TYPE_RGB32;
    try testing.expect(is_rgb24 or is_rgb32);
